...
const signupDroneTech = async (req, res, next) => {
  ...
  pool.query(
    ...
    (err, result) => {
      if (err)
        return next(
          new HttpError("Input chainName or storeName does not exist", 404)
        );
      return res.status(201).json({ success: true });
    }
  );
...
const signUpManager = async (req, res, next) => {
  ...
  pool.query(
    sql,
    [username, password, fname, lname, street, city, state, zipcode, chainName],
    (err, result) => {
      if (err) {
        return next(
          new HttpError(
            "Incorrect chain name or chainname has already been assigned a manager",
            404
          )
        );
      }
...
const login = async (req, res, next) => {
  ...
  pool.query(sql, [username, password], (err, result) => {
    ...
    if (count == 1) {
      ...
      // response
      return res.status(201).json({ success: true, information: info });
    } else {
      return next(
        new HttpError(
          "Wrong Credentials or User does not exist in the database",
          401
        )
      );
    }
